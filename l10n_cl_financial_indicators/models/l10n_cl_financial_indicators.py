...
class ResCurrency(models.Model):
    ...
    rounding = fields.Float(
        string="Rounding Factor", default=0.01, digits=dp.get_precision("Currency")
    )
    ...
    def update_currency(self, indic, currency_id=False):
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                _logger.info(" currency: %s result: %s" % (currency_id, data_json))
            ...
            _logger.info("Currency %s not available: %s" % (indic[0], r.status_code))

    def action_update_currency(self):
        indic = [self.name, [indicators[self.name][0], indicators[self.name][1]]]
